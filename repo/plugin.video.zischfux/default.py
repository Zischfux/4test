...
if addon.getSetting('custom_path_enable') == "true" and addon.getSetting('custom_path') != "":
	...
	if not os.path.exists(txtpath):
		os.makedirs(txtpath)
txtfile = txtpath + "Zuletzt Gesehen.json"
...
def send2starmovies(line):
	if enable_debug	== "true": xbmc.log("<<<plugin.video.zischfux (starmovies) "+str(line), 3)
	wid = 0
	...
	if enable_debug	== "true": xbmc.log("<<<plugin.video.zischfux (starmovies) "+url, 3)
	try:
...
def videoEnd():
	...
	if enable_debug	== "true": xbmc.log("<<<plugin.video.zischfux (end source) "+xsource, 3)
	...
	for line in lines:
		...
		if replay == "S":
			...
			lines.insert(0, line)
			replay = "S"
			if enable_debug	== "true": xbmc.log("<<<plugin.video.zischfux (end final replay) "+str(line), 3)
	...
	if replay=="N":
		...
		if enable_debug	== "true": xbmc.log("<<<plugin.video.zischfux (end final play) "+str(newline), 3)
...
class KodiPlayer(xbmc.Player):
	...
	def onPlayBackStarted(self):
		if xbmc.getCondVisibility('Player.HasVideo'):
			...
			result, data = JSquery(request)[:2]
			item=data["item"]
			if enable_debug	== "true": xbmc.log("<<<plugin.video.zischfux (start play) "+str(item), 3)
